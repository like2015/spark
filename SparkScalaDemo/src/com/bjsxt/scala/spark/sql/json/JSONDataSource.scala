package com.bjsxt.scala.spark.sql.json
...
object JSONDataSource {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 创建学生成绩DataFrame
    val studentScoresDF = sqlContext.read.json("hdfs://hadoop1:9000/input/score.json")
    ...
    // 创建学生基本信息DataFrame
    val studentInfoJSONs = Array("{\"name\":\"Michael\", \"age\":18}", 
        "{\"name\":\"Andy\", \"age\":17}",
        "{\"name\":\"Justin\", \"age\":19}")
    ...
    val goodStudentsRDD = 
        goodStudentScoresDF.map { row => (row.getAs[String]("name"), row.getAs[Long]("score")) }
            .join(goodStudentInfosDF.map { row => (row.getAs[String]("name"), row.getAs[Long]("age")) })  
